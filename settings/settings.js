document.addEventListener('DOMContentLoaded', () => {
  ...
  // Custom crosshair: visible on Cosmic pages, pointer-events disabled so it never blocks clicks.
  ...
  function drawCrosshair(){let el=document.getElementById('cosmic-crosshair');if(!el){el=document.createElement('div');el.id='cosmic-crosshair';document.body.appendChild(el);Object.assign(el.style,{position:'fixed',left:'50%',top:'50%',transform:'translate(-50%,-50%)',zIndex:'99998',pointerEvents:'none',font:'700 24px/1 system-ui,sans-serif',textAlign:'center'});}const [symbol,color]=CROSSHAIRS[crosshair.value]||CROSSHAIRS.original;el.textContent=symbol;el.style.color=color;el.style.textShadow=color==='transparent'?'none':`0 0 7px ${color}`;}
  crosshair.value=localStorage.getItem('cosmic-crosshair')||'original';drawCrosshair();crosshair.addEventListener('change',()=>{localStorage.setItem('cosmic-crosshair',crosshair.value);drawCrosshair();});
  ...
  const cloak=document.getElementById('cloakSelect');cloak.value=localStorage.getItem('savedCloak')||'none';cloak.addEventListener('change',()=>{if(cloak.value==='none')localStorage.removeItem('savedCloak');else localStorage.setItem('savedCloak',cloak.value);});
  ...
  window.addEventListener('keydown',e=>{const key=localStorage.getItem('cosmic-panic-key'),url=localStorage.getItem('cosmic-panic-url');if(key&&url&&e.key===key&&document.activeElement!==panicKey&&document.activeElement!==panicUrl)window.location.href=url;});
