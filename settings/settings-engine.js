(function () {
  ...
  const originalTitle=document.title||'Cosmic';
  const presets={google:['Google Search','#4285f4'],drive:['Google Drive','#0f9d58'],classroom:['Google Classroom','#1967d2'],canvas:['Canvas','#e66000']};
  ...
  function applyTheme(name){
    const key=Object.prototype.hasOwnProperty.call(THEMES,name)?name:'default',t=THEMES[key];
    localStorage.setItem('cosmic-theme',key); root.dataset.cosmicTheme=key;
    root.style.setProperty('--cosmic-bg',t.bg); root.style.setProperty('--cosmic-accent',t.accent); root.style.setProperty('--accent',t.accent);
    root.style.setProperty('--card-border',t.accent+'66'); root.style.setProperty('--card-hover',t.accent+'22'); root.style.setProperty('--bg',t.bg);
    const body=document.body;
    if(body){body.dataset.cosmicTheme=key;body.style.setProperty('--cosmic-bg',t.bg);body.style.setProperty('--cosmic-accent',t.accent);body.style.setProperty('--accent',t.accent);}
  ...
  function applyBackground(){
    ...
    const bg=getComputedStyle(root).getPropertyValue('--cosmic-bg').trim()||'#061410';
    ...
    const x=body.style.getPropertyValue('--cosmic-mx')||'20%';
    const y=body.style.getPropertyValue('--cosmic-my')||'20%';
    const layers=[];
    ...
    body.style.setProperty('background-attachment',image?'fixed':'scroll','important');
    body.style.setProperty('background-size',image?'cover':'auto','important');
  ...
  function applyCloak(){
    const cloak=get('savedCloak','none'); if(!document.head)return;
    ...
    if(!cloakObserver){
      cloakObserver=new MutationObserver(()=>{
        const activeCloak=get('savedCloak','none');
        if(activeCloak==='none'||!presets[activeCloak])return;
        const active=document.querySelector('link[data-cosmic-cloak="true"]');
        if(!active)applyCloak();
        else if(document.title!==presets[activeCloak][0])document.title=presets[activeCloak][0];
      });
  ...
  function applyCrosshair(){
    ...
    if(selected==='none'){if(el)el.remove();return;}
    if(!el){el=document.createElement('div');el.id='cosmic-crosshair';body.appendChild(el);Object.assign(el.style,{position:'fixed',left:'50%',top:'50%',transform:'translate(-50%,-50%)',zIndex:'2147483646',pointerEvents:'none',font:'700 24px/1 system-ui,sans-serif',textAlign:'center'});}
    const [symbol,color]=CROSSHAIRS[selected]||CROSSHAIRS.original;el.textContent=symbol;el.style.color=color;el.style.textShadow=`0 0 7px ${color}`;
  }

  function ensureMusicElement(){
    const body=document.body;if(!body)return null;
    let audio=document.getElementById('cosmic-background-audio');
    if(!audio){audio=document.createElement('audio');audio.id='cosmic-background-audio';audio.loop=true;audio.preload='auto';audio.setAttribute('aria-hidden','true');audio.style.display='none';body.appendChild(audio);}
    return audio;
  }

  function applyMusic(){
    const audio=ensureMusicElement();if(!audio)return;
    ...
    if(audio.dataset.cosmicSrc!==src){audio.dataset.cosmicSrc=src;audio.src=src;if(src)audio.load();}
    if(!enabled||!src){audio.pause();return;}
    if(musicReady)audio.play().catch(()=>{});
  }

  function enableMusicGesture(){if(musicReady)return;musicReady=true;applyMusic();document.removeEventListener('pointerdown',enableMusicGesture,true);document.removeEventListener('keydown',enableMusicGesture,true);}
  ...
  function applyPanicShortcut(){
    ...
    window.__cosmicPanicHandler=e=>{const key=localStorage.getItem('cosmic-panic-key'),url=localStorage.getItem('cosmic-panic-url');if(!key||!url||e.key!==key)return;const target=document.activeElement;if(target&&['INPUT','TEXTAREA','SELECT'].includes(target.tagName))return;e.preventDefault();e.stopPropagation();try{window.top.location.assign(url);}catch{window.location.assign(url);}};
  ...
  function installStyle(){
    ...
    const style=document.createElement('style');style.id='cosmic-settings-engine-style';
    style.textContent=`:root{--cosmic-bg:#061410;--cosmic-accent:#2dccff;--accent:#2dccff}.cosmic-rgb-theme{animation:cosmicRgb 7s linear infinite}@keyframes cosmicRgb{0%{--cosmic-accent:#ff0055;--accent:#ff0055}33%{--cosmic-accent:#00ff88;--accent:#00ff88}66%{--cosmic-accent:#0088ff;--accent:#0088ff}100%{--cosmic-accent:#ff0055;--accent:#ff0055}}body.cosmic-interactive-bg{background-color:var(--cosmic-bg)!important}.cosmic-study-cloak-disabled #educational-cloak{display:none!important}body.cosmic-custom-background{background-color:var(--cosmic-bg)!important}`;
    (document.head||document.documentElement).appendChild(style);
  }

  function applyAll(){applyTheme(get('cosmic-theme','default'));applyBackground();applyCloak();applyStudyCloak();applyCrosshair();applyMusic();applyPanicShortcut();}
